...
function getById(id) {
    const expense = gExpenses.find((expense) => expense._id === id);
    return Promise.resolve({ ...expense });
}

function save(expenseToSave) {
    if (expenseToSave._id) {
        const idx = gExpenses.findIndex(
            (expense) => expense._id === expenseToSave._id
        );
        gExpenses.splice(idx, 1, expenseToSave);
    } else {
        expenseToSave._id = makeId();
        expenseToSave.createdAt = Date.now();
        gExpenses.push(expenseToSave);
    }
    storageService.store(STORAGE_KEY, gExpenses);
    return Promise.resolve(expenseToSave);
}
